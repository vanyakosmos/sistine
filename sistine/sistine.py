...
def find_touch_point(contour, x, y, w, h):
    buf = np.zeros((h, w))
...
def find_hover_point(contour_big, x1, y1, w1, h1, contour_small, x2, y2, w2, h2):
    ...
    if len(byarea) > 2:
        ...
        if largest_area > MIN_FINGER_SIZE:
            ...
            if overlap and y2 + h2 < y1 and smaller_area / largest_area >= REFLECTION_MIN_RATIO:
                ...
                hover_x, hover_y = find_hover_point(
                ...
                touch_x, touch_y, wloc, width = find_touch_point(largest_contour, x1, y1, w1, h1)
...
def main_loop(segmented, debugframe, options, ticks, drawframe, calib, state):
    ...
    if 'hom' not in calib:
        ...
        hom = find_transform(webcam_points, screen_points)
        calib['hom'] = hom
    ...
    if not options['nocalib']:
        for i, j in calib['orp']:
            i_, j_ = apply_transform(i, j, np.linalg.inv(calib['hom']))
    ...
    if touch is not None:
        if not options['demo']:
            cv2.circle(drawframe, (x, y), CIRCLE_RADIUS, PURPLE, -1)
        x_, y_ = apply_transform(x, y, calib['hom'])
...
def find_transform(webcam_points, screen_points):
    print(webcam_points)
    print(screen_points)
...
def apply_transform(x, y, homography):
...
def run():
    ...
    if 'nocalib' in sys.argv:
        ...
        stages = [calibration(i) for i in range(7)] + [main_loop]
